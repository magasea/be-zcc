...
# part 2 for new developing db
echo '#!/bin/bash' > ./commands.sh
echo 'cd /home/chenwei/tmp' >> ./commands.sh
echo 'mysqldump --compatible=ansi -u root -pwensheng -B ZCC_WECHAT > ./zccwechat-`date +%Y-%m-%d`.sql' >> ./commands.sh
...
ssh chenwei@10.20.100.238 "cd /home/chenwei/tmp/mysqlback/ && mysql -u root -pWensheng@12345678 < ./zccwechat-`date +%Y-%m-%d`.sql"
#

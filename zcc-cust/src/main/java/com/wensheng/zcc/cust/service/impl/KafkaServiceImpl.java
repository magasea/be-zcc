...
//      custTrdCmpy.setCmpyNameUpdate("-1");
    ...
    log.info("查询爬虫基础库中信息url:{}, crawlResultDTO:{}", url, crawlResultDTO);
    if(null == crawlResultDTO || CollectionUtils.isEmpty(crawlResultDTO.getList())){
      log.info("查询爬虫基础库中信息,暂无信息");
      return;
    }
    //此场景只有一个
    for (CmpyBasicBizInfoSync cmpyBasicBizInfoSyncCheck: crawlResultDTO.getList()) {
      if(cmpyBizInfoResult.getCmpyName().equals(cmpyBasicBizInfoSyncCheck.getName())){
        cmpyBasicBizInfoSync = cmpyBasicBizInfoSyncCheck;
        break;
      }
    }
